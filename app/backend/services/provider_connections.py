...
# ------------------ Service Layer ------------------
class Provider_connectionsService:
    """Service layer for Provider_connections operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Provider_connections]:
        """Create a new provider_connections"""
        try:
            obj = Provider_connections(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created provider_connections with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating provider_connections: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Provider_connections]:
        """Get provider_connections by ID"""
        try:
            query = select(Provider_connections).where(Provider_connections.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching provider_connections {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of provider_connectionss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Provider_connections, field):
                        column = getattr(Provider_connections, field)
            ...
            stmt = select(Provider_connections, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Provider_connections, field_name):
                        stmt = stmt.order_by(getattr(Provider_connections, field_name).desc())
                else:
                    if hasattr(Provider_connections, sort):
                        stmt = stmt.order_by(getattr(Provider_connections, sort))
            else:
                stmt = stmt.order_by(Provider_connections.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Provider_connections)
                for condition in conditions:
            ...
            logger.error(f"Error fetching provider_connections list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Provider_connections]:
        """Update provider_connections"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Provider_connections {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated provider_connections {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating provider_connections {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Provider_connections {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted provider_connections {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting provider_connections {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Provider_connections]:
        """Get provider_connections by any field"""
        try:
            if not hasattr(Provider_connections, field_name):
                raise ValueError(f"Field {field_name} does not exist on Provider_connections")
            column = getattr(Provider_connections, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Provider_connections).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching provider_connections by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Provider_connections]:
        """Get list of provider_connectionss filtered by field"""
        try:
            if not hasattr(Provider_connections, field_name):
                raise ValueError(f"Field {field_name} does not exist on Provider_connections")
            column = getattr(Provider_connections, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Provider_connections.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching provider_connectionss by {field_name}: {str(e)}")
            raise