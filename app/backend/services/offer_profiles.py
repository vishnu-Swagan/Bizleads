...
# ------------------ Service Layer ------------------
class Offer_profilesService:
    """Service layer for Offer_profiles operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Offer_profiles]:
        """Create a new offer_profiles"""
        try:
            obj = Offer_profiles(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created offer_profiles with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating offer_profiles: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Offer_profiles]:
        """Get offer_profiles by ID"""
        try:
            query = select(Offer_profiles).where(Offer_profiles.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching offer_profiles {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of offer_profiless"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Offer_profiles, field):
                        column = getattr(Offer_profiles, field)
            ...
            stmt = select(Offer_profiles, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Offer_profiles, field_name):
                        stmt = stmt.order_by(getattr(Offer_profiles, field_name).desc())
                else:
                    if hasattr(Offer_profiles, sort):
                        stmt = stmt.order_by(getattr(Offer_profiles, sort))
            else:
                stmt = stmt.order_by(Offer_profiles.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Offer_profiles)
                for condition in conditions:
            ...
            logger.error(f"Error fetching offer_profiles list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Offer_profiles]:
        """Update offer_profiles"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Offer_profiles {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated offer_profiles {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating offer_profiles {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete offer_profiles"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Offer_profiles {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted offer_profiles {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting offer_profiles {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Offer_profiles]:
        """Get offer_profiles by any field"""
        try:
            if not hasattr(Offer_profiles, field_name):
                raise ValueError(f"Field {field_name} does not exist on Offer_profiles")
            column = getattr(Offer_profiles, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Offer_profiles).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching offer_profiles by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Offer_profiles]:
        """Get list of offer_profiless filtered by field"""
        try:
            if not hasattr(Offer_profiles, field_name):
                raise ValueError(f"Field {field_name} does not exist on Offer_profiles")
            column = getattr(Offer_profiles, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Offer_profiles)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Offer_profiles.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching offer_profiless by {field_name}: {str(e)}")
            raise