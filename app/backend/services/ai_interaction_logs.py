...
# ------------------ Service Layer ------------------
class Ai_interaction_logsService:
    """Service layer for Ai_interaction_logs operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Ai_interaction_logs]:
        """Create a new ai_interaction_logs"""
        try:
            if user_id:
                data['user_id'] = user_id
            obj = Ai_interaction_logs(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created ai_interaction_logs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating ai_interaction_logs: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for ai_interaction_logs {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Ai_interaction_logs]:
        """Get ai_interaction_logs by ID (user can only see their own records)"""
        try:
            query = select(Ai_interaction_logs).where(Ai_interaction_logs.id == obj_id)
            if user_id:
                query = query.where(Ai_interaction_logs.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching ai_interaction_logs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of ai_interaction_logss (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Ai_interaction_logs.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Ai_interaction_logs, field):
                        column = getattr(Ai_interaction_logs, field)
            ...
            stmt = select(Ai_interaction_logs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Ai_interaction_logs, field_name):
                        stmt = stmt.order_by(getattr(Ai_interaction_logs, field_name).desc())
                else:
                    if hasattr(Ai_interaction_logs, sort):
                        stmt = stmt.order_by(getattr(Ai_interaction_logs, sort))
            else:
                stmt = stmt.order_by(Ai_interaction_logs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Ai_interaction_logs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching ai_interaction_logs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Ai_interaction_logs]:
        """Update ai_interaction_logs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Ai_interaction_logs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated ai_interaction_logs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating ai_interaction_logs {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete ai_interaction_logs (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Ai_interaction_logs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted ai_interaction_logs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting ai_interaction_logs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Ai_interaction_logs]:
        """Get ai_interaction_logs by any field"""
        try:
            if not hasattr(Ai_interaction_logs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Ai_interaction_logs")
            column = getattr(Ai_interaction_logs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Ai_interaction_logs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching ai_interaction_logs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Ai_interaction_logs]:
        """Get list of ai_interaction_logss filtered by field"""
        try:
            if not hasattr(Ai_interaction_logs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Ai_interaction_logs")
            column = getattr(Ai_interaction_logs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Ai_interaction_logs)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Ai_interaction_logs.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching ai_interaction_logss by {field_name}: {str(e)}")
            raise