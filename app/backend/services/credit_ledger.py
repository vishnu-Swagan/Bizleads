...
# ------------------ Service Layer ------------------
class Credit_ledgerService:
    """Service layer for Credit_ledger operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Credit_ledger]:
        """Create a new credit_ledger"""
        try:
            obj = Credit_ledger(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created credit_ledger with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating credit_ledger: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Credit_ledger]:
        """Get credit_ledger by ID"""
        try:
            query = select(Credit_ledger).where(Credit_ledger.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching credit_ledger {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of credit_ledgers"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Credit_ledger, field):
                        column = getattr(Credit_ledger, field)
            ...
            stmt = select(Credit_ledger, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Credit_ledger, field_name):
                        stmt = stmt.order_by(getattr(Credit_ledger, field_name).desc())
                else:
                    if hasattr(Credit_ledger, sort):
                        stmt = stmt.order_by(getattr(Credit_ledger, sort))
            else:
                stmt = stmt.order_by(Credit_ledger.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Credit_ledger)
                for condition in conditions:
            ...
            logger.error(f"Error fetching credit_ledger list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Credit_ledger]:
        """Update credit_ledger"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Credit_ledger {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated credit_ledger {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating credit_ledger {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete credit_ledger"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Credit_ledger {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted credit_ledger {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting credit_ledger {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Credit_ledger]:
        """Get credit_ledger by any field"""
        try:
            if not hasattr(Credit_ledger, field_name):
                raise ValueError(f"Field {field_name} does not exist on Credit_ledger")
            column = getattr(Credit_ledger, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Credit_ledger).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching credit_ledger by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Credit_ledger]:
        """Get list of credit_ledgers filtered by field"""
        try:
            if not hasattr(Credit_ledger, field_name):
                raise ValueError(f"Field {field_name} does not exist on Credit_ledger")
            column = getattr(Credit_ledger, field_name)
            ...
            result = await self.db.execute(
                ...
                .offset(skip)
                .limit(limit)
                .order_by(Credit_ledger.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching credit_ledgers by {field_name}: {str(e)}")
            raise