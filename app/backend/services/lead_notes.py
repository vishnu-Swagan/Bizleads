...
# ------------------ Service Layer ------------------
class Lead_notesService:
    """Service layer for Lead_notes operations"""
    ...
    async def create(self, data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Lead_notes]:
        ...
        try:
            ...
            logger.info(f"Created lead_notes with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating lead_notes: {str(e)}")
            raise
    ...
    async def check_ownership(self, obj_id: int, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Error checking ownership for lead_notes {obj_id}: {str(e)}")
            return False

    async def get_by_id(self, obj_id: int, user_id: Optional[str] = None) -> Optional[Lead_notes]:
        """Get lead_notes by ID (user can only see their own records)"""
        try:
            query = select(Lead_notes).where(Lead_notes.id == obj_id)
            if user_id:
                query = query.where(Lead_notes.user_id == user_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching lead_notes {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of lead_notess (user can only see their own records)"""
        try:
            ...
            conditions = []
            if user_id:
                conditions.append(Lead_notes.user_id == user_id)

            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Lead_notes, field):
                        column = getattr(Lead_notes, field)
            ...
            stmt = select(Lead_notes, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Lead_notes, field_name):
                        stmt = stmt.order_by(getattr(Lead_notes, field_name).desc())
                else:
                    if hasattr(Lead_notes, sort):
                        stmt = stmt.order_by(getattr(Lead_notes, sort))
            else:
                stmt = stmt.order_by(Lead_notes.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Lead_notes)
                for condition in conditions:
            ...
            logger.error(f"Error fetching lead_notes list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any], user_id: Optional[str] = None) -> Optional[Lead_notes]:
        """Update lead_notes (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Lead_notes {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated lead_notes {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating lead_notes {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int, user_id: Optional[str] = None) -> bool:
        """Delete lead_notes (requires ownership)"""
        try:
            obj = await self.get_by_id(obj_id, user_id=user_id)
            if not obj:
                logger.warning(f"Lead_notes {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted lead_notes {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting lead_notes {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Lead_notes]:
        """Get lead_notes by any field"""
        try:
            if not hasattr(Lead_notes, field_name):
                raise ValueError(f"Field {field_name} does not exist on Lead_notes")
            column = getattr(Lead_notes, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Lead_notes).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching lead_notes by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Lead_notes]:
        """Get list of lead_notess filtered by field"""
        try:
            if not hasattr(Lead_notes, field_name):
                raise ValueError(f"Field {field_name} does not exist on Lead_notes")
            column = getattr(Lead_notes, field_name)
            ...
            logger.error(f"Error fetching lead_notess by {field_name}: {str(e)}")
            raise