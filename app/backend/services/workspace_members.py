...
class Workspace_membersService:
    ...
    async def get_list(
        ...
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Workspace_members, field):
                        column = getattr(Workspace_members, field)
            ...
            stmt = select(Workspace_members, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Workspace_members, field_name):
                        stmt = stmt.order_by(getattr(Workspace_members, field_name).desc())
                else:
                    if hasattr(Workspace_members, sort):
                        stmt = stmt.order_by(getattr(Workspace_members, sort))
            else:
                stmt = stmt.order_by(Workspace_members.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Workspace_members)
                for condition in conditions:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Workspace_members]:
        ...
        try:
            ...
            column = getattr(Workspace_members, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Workspace_members).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
