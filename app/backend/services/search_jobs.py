...
# ------------------ Service Layer ------------------
class Search_jobsService:
    """Service layer for Search_jobs operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Search_jobs]:
        """Create a new search_jobs"""
        try:
            obj = Search_jobs(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created search_jobs with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating search_jobs: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Search_jobs]:
        """Get search_jobs by ID"""
        try:
            query = select(Search_jobs).where(Search_jobs.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching search_jobs {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of search_jobss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Search_jobs, field):
                        column = getattr(Search_jobs, field)
            ...
            stmt = select(Search_jobs, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Search_jobs, field_name):
                        stmt = stmt.order_by(getattr(Search_jobs, field_name).desc())
                else:
                    if hasattr(Search_jobs, sort):
                        stmt = stmt.order_by(getattr(Search_jobs, sort))
            else:
                stmt = stmt.order_by(Search_jobs.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Search_jobs)
                for condition in conditions:
            ...
            logger.error(f"Error fetching search_jobs list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Search_jobs]:
        """Update search_jobs"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Search_jobs {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated search_jobs {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating search_jobs {obj_id}: {str(e)}")
            raise
    ...
    async def delete(self, obj_id: int) -> bool:
        ...
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Search_jobs {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted search_jobs {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting search_jobs {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Search_jobs]:
        """Get search_jobs by any field"""
        try:
            if not hasattr(Search_jobs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Search_jobs")
            column = getattr(Search_jobs, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Search_jobs).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching search_jobs by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Search_jobs]:
        """Get list of search_jobss filtered by field"""
        try:
            if not hasattr(Search_jobs, field_name):
                raise ValueError(f"Field {field_name} does not exist on Search_jobs")
            column = getattr(Search_jobs, field_name)
            ...
            logger.error(f"Error fetching search_jobss by {field_name}: {str(e)}")
            raise