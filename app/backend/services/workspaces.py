...
class WorkspacesService:
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Workspaces]:
    ...
    async def get_by_id(self, obj_id: int) -> Optional[Workspaces]:
    ...
    async def get_list(
        ...
        """Get paginated list of workspacess"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Workspaces, field):
                        column = getattr(Workspaces, field)
            ...
            stmt = select(Workspaces, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    ...
                    if hasattr(Workspaces, field_name):
                        stmt = stmt.order_by(getattr(Workspaces, field_name).desc())
                else:
                    if hasattr(Workspaces, sort):
                        stmt = stmt.order_by(getattr(Workspaces, sort))
            else:
                stmt = stmt.order_by(Workspaces.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Workspaces)
                for condition in conditions:
    ...
    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Workspaces]:
        """Update workspaces"""
        try:
    ...
    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Workspaces]:
        ...
        try:
            ...
            column = getattr(Workspaces, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Workspaces).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
