...
@router.get("/all", response_model=Lead_notesListResponse)
async def query_lead_notess_all(
    query: str = Query(None, description='Query conditions as JSON, e.g. {"id":2} or {"id":{"$gte":2}}'),
    sort: str = Query(None, description="Sort field (prefix with '-' for descending)"),
    skip: int = Query(0, ge=0, description="Number of records to skip"),
    limit: int = Query(20, ge=1, le=2000, description="Max number of records to return"),
    ...
    # Query lead_notess with filtering, sorting, and pagination without user limitation
    logger.debug(f"Querying lead_notess: query={query}, sort={sort}, skip={skip}, limit={limit}, fields={fields}")
