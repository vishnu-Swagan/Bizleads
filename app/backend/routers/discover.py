...
async def run_discovery(
    ...
    try:
        ...
        # Measure before scoring, so the score reflects the site as it is
        # rather than what could be guessed from a listing.
...
# Zero since discovery started measuring every result it returns. Measurement
# is part of what a search buys, so charging again to re-measure a lead would
# bill twice for the same work — and the endpoint now exists mainly to refresh
# a lead whose site has changed, which nobody should have to pay for.
